...
document.addEventListener('DOMContentLoaded', async function () {
    ...
    function handleAccept(index) {
        const organization = new Map(Object.entries(organizations[index]));
        organization.set("status", "accepted")
        organizations[index] = Object.fromEntries(organization);
        displayOrganizationDetails(organizations);
    }

    function handleReject(index) {
        const organization = new Map(Object.entries(organizations[index]));
        organization.set("status", "rejected")
        organizations[index] = Object.fromEntries(organization);
        displayOrganizationDetails(organizations);
    }
    ...
    organizationDetails.addEventListener('click', function (event) {
        if (event.target.classList.contains('accept-button')) {
            const organizationElement = event.target.closest('.volunteer');
            const index = Array.from(organizationElement.parentNode.children).indexOf(organizationElement);
            handleAccept(index);
        } else if (event.target.classList.contains('reject-button')) {
            const organizationElement = event.target.closest('.volunteer');
            const index = Array.from(organizationElement.parentNode.children).indexOf(organizationElement);
            handleReject(index);
        } else if (event.target.classList.contains('info-button')) {
